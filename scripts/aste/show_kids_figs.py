...
#                        help='plot Q, responsivity, tau_qp, NEP figures')
    parser.add_argument('--q', action='store_true', default=False,
                        help='plot Q, either Q or Qroom')
    parser.add_argument('--qroom', action='store_true', default=False,
                        help='plot Qroom, either Q or Qroom')
...
#    args_ar = [args.all, args.q, args.resp, args.tau, args.nep]
    args_ar = [args.q, args.qroom]
    if not (True in args_ar):
        parser.print_help()
        sys.exit()
    if args.q and args.qroom:
        parser.print_help()
        sys.exit()
    ...
    if args.q:
        infilename = 'FitSweep_fit.npy'
        outfilename0 = 'figQvalues.png'
        outfilename1 = 'figQvalues_hist.png'
    elif args.qroom:
        infilename = 'FitSweep_fit_roomchopper.npy'
        outfilename0 = 'figQvalues_roomchopper.png'
        outfilename1 = 'figQvalues_hist_roomchopper.png'
    
    from util import weighted_avg_and_std, delete_elements
    if args.q or args.qroom:
        ifile = os.path.join(outdir, infilename)
...
#        print 'by Qi: ', kidid[idx_ar]
        ...
        if args.save: plt.savefig(os.path.join(outdir, outfilename0))
        ...
        if args.save: plt.savefig(os.path.join(outdir, outfilename1))

...
def main(argv=None):
    ...
    if args.mode=='1' or args.mode=='all':
        NCPU = args.ncpu
        first_ana(kids, reftxtlist, rebin, args.force, NCPU)
...
def first_ana(kids, reftxtlist, rebin, force, NCPU):
    ...
    try:
        ...
        print("readtxt = my_loadtxt(reftxtlist, unpack=True, skiprows=1):: failed (reference.dat is void?)")
        reflist = []
    #print reflist
    ...
    for i, kid in kids.items():
        ...
        params_list.append({"i": i, "kid": kid, "plotdir": plotdir, "rebin": rebin, "reftxtlist": reftxtlist, "reflist": reflist})
...
def second_ana(kids, reftxtlist, refidx, rebin, force):
    ...
    #Fsubmm, Spectrum, result_fit, Spectrum_df, result_fit_df = np.array([Fsubmm, Spectrum, result_fit, Spectrum_df, result_fit_df], dtype=object)
    result_fit_list = copy.deepcopy(result_fit)
    result_fit_df_list = copy.deepcopy(result_fit_df)
    Fsubmm = np.array(Fsubmm)
    Spectrum = np.array(Spectrum)
    result_fit = np.array(result_fit)
    Spectrum_df = np.array(Spectrum_df)
    result_fit_df = np.array(result_fit_df)
    ...
    np.save(outdir+'/AnaSpectrum2nd_fit.npy',
            np.array([kidid, Fsubmm.tolist(), Spectrum.tolist(), result_fit.tolist(), Spectrum_df.tolist(), result_fit_df.tolist()], dtype=object))
    #        np.array([kidid, Fsubmm.tolist(), Spectrum.tolist(), result_fit.tolist(), Spectrum_df.tolist(), result_fit_df.tolist()]))
    result_fit = copy.deepcopy(result_fit_list)
    result_fit_df = copy.deepcopy(result_fit_df_list)
    for i, ind in enumerate(sorted(lrefidx)):
        del result_fit[ind-i]
        del result_fit_df[ind-i]
    with open(outdir+'/result_fit.pkl', 'wb') as f:
        pickle.dump(result_fit, f)
    with open(outdir+'/result_fit_df.pkl', 'wb') as f:
        pickle.dump(result_fit_df, f)
